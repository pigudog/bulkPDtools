...
```{r}
load("GSE155489.Rdata")
group_list1=c(rep('CT',4),rep('PC',4))
...
```{r}
save(DEG2,file = "deg/DESeq2_deg_data2.Rdata")
write.table(DEG2,row.names=TRUE,col.names=TRUE,sep=",","deg/DESeq2_degPCvsCT_data2.csv")
```
...
pheatmap(n,show_colnames =F,show_rownames = F,
         annotation_col=ac,filename = './check2/heatmap2_top1000_sd.png')
```
```{r}
#DEG
DEG <- DEG2
...
# h
cg <- c(tem1,tem2)
anno=data.frame(row.names = colnames(dat),group=c(rep('CT',4),rep('PC',4)))
...
```
```{r}
mitodeg2 <- intersect_gene
save(mitodeg2,file = "mito/mitodeg2.Rdata")
write.table(mitodeg2,file ='mito/mitodeg2.txt',row.names = F,col.names = F,quote=F)
```
...
length(intersect_gene)
mitodeg2_up <- intersect_gene
save(mitodeg3_up,file = "mito/mitodeg2_up.Rdata")
write.table(mitodeg2_up,file ='mito/mitodeg2_up.txt',row.names = F,col.names = F,quote=F)
```
```{r}
...
length(intersect_gene)
mitodeg2_down <- intersect_gene
save(mitodeg2_down,file = "mito/mitodeg2_down.Rdata")
write.table(mitodeg2_down,file ='mito/mitodeg2_down.txt',row.names = F,col.names = F,quote=F)
```
